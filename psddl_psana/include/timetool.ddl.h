...
public:
  ...
  virtual ~ConfigV3();
  /** Time Axis of Image */
  virtual TimeTool::ConfigV3::Axis project_axis() const = 0;
  /** Use Full Regions for Background Subtraction */
  virtual uint8_t use_full_roi() const = 0;
  /** Use Fit instead of FIR for edge finding */
  virtual uint8_t use_fit() const = 0;
  /** Record Raw Image into Event */
  virtual uint8_t write_image() const = 0;
  /** Record Time Axis Projections into Event */
  virtual uint8_t write_projections() const = 0;
  ...
  virtual uint8_t fit_params_dim() const = 0;
  /** Length of EPICS PV base name */
  virtual uint8_t base_name_length() const = 0;
  /** Number of Beam Logic Event Codes */
  virtual uint16_t number_of_beam_event_codes() const = 0;
  ...
  virtual uint32_t fit_max_iterations() const = 0;
  /** Factor for Scaling Uncertainty for Fit Weights */
  virtual double fit_weights_factor() const = 0;
  /** Signal Region Coordinates Start */
  virtual const Camera::FrameCoord& sig_roi_lo() const = 0;
  /** Signal Region Coordinates End */
  virtual const Camera::FrameCoord& sig_roi_hi() const = 0;
  ...
  virtual ndarray<const double, 1> fit_params() const = 0;
  /** EPICS PV base name */
  virtual const char* base_name() const = 0;
  /** Size of projections */
  virtual uint32_t signal_projection_size() const = 0;
  virtual uint32_t sideband_projection_size() const = 0;
  virtual uint32_t reference_projection_size() const = 0;
  virtual uint32_t signal_x_size() const = 0;
  virtual uint32_t signal_y_size() const = 0;
  virtual uint32_t signal_size() const = 0;
  virtual uint32_t sideband_x_size() const = 0;
  virtual uint32_t sideband_y_size() const = 0;
  virtual uint32_t sideband_size() const = 0;
  virtual uint32_t reference_x_size() const = 0;
  virtual uint32_t reference_y_size() const = 0;
  virtual uint32_t reference_size() const = 0;
  /** Method which returns the shape (dimensions) of the data returned by base_name() method. */
  virtual std::vector<int> base_name_shape() const = 0;
};
...
public:
  ...
  virtual ~DataV3();
  /** Event designation */
  virtual TimeTool::DataV3::EventType event_type() const = 0;
  /** Amplitude of the edge */
  virtual double amplitude() const = 0;
  /** Filtered pixel position of the edge */
  virtual double position_pixel() const = 0;
  ...
  virtual ndarray<const int32_t, 1> projected_reference() const = 0;
  /** Signal */
  virtual ndarray<const int32_t, 2> full_signal() const = 0;
  /** Sideband */
  virtual ndarray<const int32_t, 2> full_sideband() const = 0;
  /** Reference */
  virtual ndarray<const int32_t, 2> full_reference() const = 0;
};
