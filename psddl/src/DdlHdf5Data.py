...
class DdlHdf5Data ( object ) :
    ...
    def parseTree(self, model):
        ...
        print("#include \"%s\"" % inc, file=self.inc)

        # add necessary includes
        print("#include \"hdf5pp/Group.h\"", file=self.inc)
        print("#include \"hdf5pp/Type.h\"", file=self.inc)
        print("#include \"PSEvt/Proxy.h\"", file=self.inc)
        print("#include \"hdf5pp/ArrayType.h\"", file=self.cpp)
        print("#include \"hdf5pp/CompoundType.h\"", file=self.cpp)
        print("#include \"hdf5pp/EnumType.h\"", file=self.cpp)
        print("#include \"hdf5pp/VlenType.h\"", file=self.cpp)
        print("#include \"hdf5pp/Utils.h\"", file=self.cpp)
        print("#include \"PSEvt/DataProxy.h\"", file=self.cpp)
        ...
        for header in inc_headers:
            if header: print(T('#include "$header"')(locals()), file=self.inc)
        for header in cpp_headers:
            if header: print(T('#include "$header"')(locals()), file=self.cpp)

        if self.top_pkg : 
            ns = "namespace %s {" % self.top_pkg
            print(ns, file=self.inc)
            print(ns, file=self.cpp)

        # enums for constants
        for const in model.constants() :
            if not const.included :
                print(_TEMPL('const_decl').render(const=const), file=self.inc)

        # regular enums
        for enum in model.enums() :
            if not enum.included :
                print(_TEMPL('enum_decl').render(enum=enum), file=self.inc)
    ...
    def _parseType(self, type):
        ...
        for config in configs:
            ...
            print(_TEMPL('make_proxy_decl').render(locals()), file=self.inc)
            print(_TEMPL('make_proxy_impl').render(locals()), file=self.cpp)

        # generate store method declaration
        print(_TEMPL('store_decl').render(locals()), file=self.inc)
        
        versions = sorted(schema.version for schema in type.h5schemas)
        max_version = versions[-1]
        print(_TEMPL('store_impl').render(locals()), file=self.cpp)
            ...
            for ds in schema.datasets:
                ...
                if ds.attributes:
                    ...
                    for attr in ds.attributes:
                        ...
                        print('    @attribute {0.name}'.format(attr), end=' ')
                        if attr.method != attr.name: print('[[method({0.method})]]'.format(attr.method), end=' ')
                    ...
                    print('  @dataset {0.name}'.format(ds), end=' ')
                    for tag, val in ds.tags.items():
                        if val is None:
                            print('[[{0}]]'.format(tag), end=' ')
                        else:
