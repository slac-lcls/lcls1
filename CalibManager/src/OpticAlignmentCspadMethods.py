...
#---------- GRAPHICS --------------
    ...
    def drawOpticalAlignmentFile(self):
        ...
        if self.nquads == 1 and self.nsegms == 2:
            ...
            axes.set_xlim((-100,1800))
            axes.set_ylim((-100,1800))
...
def input_option_parser(d_dir, d_fname='optical_metrology.txt', d_pbits=0o177777, d_gbits=0o377, d_n90=0):

    h_dir   = 'directory for metrology files, default = %s' % d_dir
    h_fname = 'metrology file name, default = %s' % d_fname
    h_pbits = 'print control bitword, default = %s' % oct(d_pbits)
    h_gbits = 'graphics control bitword, default = %s' % oct(d_gbits)
    h_n90   = 'detector orientation in number of times 90 degree (works for 2x2 and v2_xpp), default = %d' % d_n90

    parser = OptionParser(description='Optical metrology processint for CSPAD - CXI', usage ='usage: %prog [options] args')
    parser.add_option('-d', '--dir',   dest='dir',    default=d_dir,   action='store', type='string', help=h_dir)
    parser.add_option('-f', '--fname', dest='fname',  default=d_fname, action='store', type='string', help=h_fname)
    parser.add_option('-p', '--pbits', dest='pbits',  default=d_pbits, action='store', type='int',    help=h_pbits)
    parser.add_option('-g', '--gbits', dest='gbits',  default=d_gbits, action='store', type='int',    help=h_gbits)
    parser.add_option('-n', '--n90',   dest='n90',    default=d_n90,   action='store', type='int',    help=h_n90)
