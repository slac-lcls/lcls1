...
class GUIDarkList(QtWidgets.QWidget) :
    ...
    def connectToThreadWorker(self):
        try : cp.thread_check_new_xtc_files.update['QString'].connect(self.signalReciever)
        except : logger.warning('connectToThreadWorker is failed', __name__)


    def disconnectFromThreadWorker(self):
        try : cp.thread_check_new_xtc_files.update['QString'].disconnect(self.signalReciever)
        except : pass
