...
SmlDataFileReader::SmlDataFileReader() : _fdXtcIndex(-1), _iCurL1Node(-1)
{
}

SmlDataFileReader::~SmlDataFileReader()
{
  close();
}

int SmlDataFileReader::open(const char* sXtcIndex)
{
  close();
  ...
  if ( _fdXtcIndex == -1 )
  {
    printf( "SmlDataFileReader::open(): Open index file %s failed, error = %s\n",
  ...
  if ( iError == -1 )
  {
    printf( "SmlDataFileReader::open(): read file header version failed (%s)\n", strerror(errno) );
    return 2;
  }
  
  if ( typeId.id() != TypeId::Id_SmlData  )
  {
    printf( "SmlDataFileReader::open(): Unsupported xtc type: %s V%d\n",
   ...
   */
  if ( (int) typeId.version() != SmlDataFileHeaderType::iXtcSmlDataVersion )
  {
    if ( (int) typeId.version() == 1 )
    {
      SmlDataFileHeaderV1 fileHeaderV1;
      lseek64(_fdXtcIndex, 0, SEEK_SET);
      int iError = ::read(_fdXtcIndex, &fileHeaderV1, sizeof(SmlDataFileHeaderV1) );
      if ( iError == -1 )
      {
        printf( "SmlDataFileReader::open(): read file header failed (%s)\n", strerror(errno) );
        return 4;
      }    
      
      new (&_fileHeader) SmlDataFileHeaderType(fileHeaderV1);
  ...
  ////!! original
  //int iRead = ::read(_fdXtcSmlData, &_fileHeader, sizeof(_fileHeader));
  //if ( iRead != sizeof(_fileHeader) )
  //{
  //  printf( "SmlDataFileReader::open(): Read header failed. Read %d bytes (expected %d), error = %s\n",
  ...
  {
    printf("SmlDataFileReader::open(): Seek to CalibCycle offset 0x%Lx failed, result = 0x%Lx\n", 
  ...
  if (_fileHeader.iNumCalib > 0)
  {
    ...
    if (iRead != iReadSize)
    {
      printf( "SmlDataFileReader::open(): Read calib cycle (%d) failed. Read %d bytes (expected %d), error = %s\n",
  ...
  if (_fileHeader.iNumEvrEvents > 0)
  {
    ...
    if (iRead != iReadSize)
    {
      printf( "SmlDataFileReader::open(): Read evr events (%d) failed. Read %d bytes (expected %d), error = %s\n",
  ...
  if (_fileHeader.iNumDetector > 0)
  {
    ...
    for ( int iDetector = 0; iDetector < _fileHeader.iNumDetector; ++iDetector )
    {
      ...
      if (iRead != iReadSize)
      {
        printf( "SmlDataFileReader::open(): Read detector %d procInfo failed. Read %d bytes (expected %d), error = %s\n",
      ...
      if (iRead != iReadSize)
      {
        printf( "SmlDataFileReader::open(): Read detector src# for detector %d failed. Read %d bytes (expected %d), error = %s\n",
          iDetector, iRead, iReadSize, strerror(errno) );
        return 6;
      }  
      ...
      if ( iRead != iReadSize)
      {
        printf( "SmlDataFileReader::open(): Read src list for detector %d failed. Read %d bytes (expected %d), error = %s\n",
          iDetector, iRead, iReadSize, strerror(errno) );
        return 7;
      }              
      ...
      if ( iRead != iReadSize)
      {
        printf( "SmlDataFileReader::open(): Read type list for detector %d failed. Read %d bytes (expected %d), error = %s\n",
          iDetector, iRead, iReadSize, strerror(errno) );
        return 8;
      }              
    } // for ( int iDetector = 0; iDetector < _fileHeader.iNumDetector; ++iDetector )
...
int SmlDataFileReader::close()
{
  ...
  if ( iError != 0 )
  {
    printf( "SmlDataFileReader::close(): Close index file failed, error = %s\n",
      strerror(errno) );
    return 1;
  }      
...
bool SmlDataFileReader::isValid() const
{
  return (_fdXtcIndex != -1);
}

int SmlDataFileReader::numL1Event(int& iNumL1Event) const
{
  iNumL1Event = -1;
  if (_fdXtcIndex == -1)
    return 1;       
...
int SmlDataFileReader::numL1EventInCalib(int iCalib, int& iNumL1Event) const
{
  iNumL1Event = -1;
  if (_fdXtcIndex == -1)
    return 1;
  ...
  {
    printf( "SmlDataFileReader::numL1EventInCalib(): Invalid Calib Cyle #%d (Max # = %d)\n",
...
int SmlDataFileReader::eventCalibToGlobal(int iCalib, int iEvent, int& iGlobalEvent) const
{
  if (_fdXtcIndex == -1)
    return 1;
  ...
  if ( iCalib < -1 || iCalib >= (int) _lCalib.size() )
  {
    printf( "SmlDataFileReader::eventCalibToGlobal(): Invalid Calib# %d (Max # = %d)\n",
      iCalib, (int) _lCalib.size()-1 );
    return 2;
  }   
  ...
  {
    printf( "SmlDataFileReader::eventCalibToGlobal(): Invalid Event# %d in Calib# %d (Max # = %d)\n",
...
int SmlDataFileReader::eventGlobalToCalib(int iGlobalEvent, int& iCalib, int& iEvent) const
{
  if (_fdXtcIndex == -1)
    return 1;
  ...
  {
    printf( "SmlDataFileReader::eventGlobalToCalib(): Invalid global Event# %d (Max # = %d)\n",
  ...
  printf( "SmlDataFileReader::eventGlobalToCalib(): Cannot find correct Calib Cycle for global Event# %d (Max # = %d)n",
    iGlobalEvent, _fileHeader.iNumIndex-1);
  return 3;
}

int SmlDataFileReader::eventTimeToGlobal(uint32_t uSeconds, uint32_t uNanoseconds, int& iGlobalEvent, bool& bExactMatch, bool& bOvertime)
{  
  iGlobalEvent  = -1;
  bExactMatch   = false;
  bOvertime     = false;
  ...
  else if ( _fileHeader.iNumIndex == 1 )
  {
    ...
    if (iError != 0)
    {
      printf("SmlDataFileReader::eventTimeToGlobal(): Cannot get time for global event# %d\n", iEventTest);
      return 2;
    }
...
int SmlDataFileReader::eventTimeToCalib(uint32_t uSeconds, uint32_t uNanoseconds, int& iCalib, int& iEvent, bool& bExactMatch, bool& bOvertime)
{
  iCalib      = -1;
  iEvent      = -1;
  bExactMatch = false;
...
int SmlDataFileReader::eventNextFiducialToGlobal(uint32_t uFiducial, int iFromGlobalEvent, int& iGlobalEvent)
{
  ...
  //printf("SmlDataFileReader::eventNextFiducialToGlobal(): fiducial 0x%x  eventFrom %d\n", uFiducial, iFromGlobalEvent); //!!debug
  ...
  for (int iEvent = 0; iEvent < NUM_FIDUCIAL_SAMPLES; ++iEvent)
  {
    ...
    if (iError != 0)
    {
      printf("SmlDataFileReader::eventNextFiducialToGlobal(): Cannot get fiducial for global event# %d\n", iFromGlobalEvent + iEvent);
      return 3;
    }
    
    //printf("SmlDataFileReader::eventNextFiducialToGlobal(): fiducial[%d] 0x%x\n", iFromGlobalEvent + iEvent, uFiducialSample[iEvent]); //!!debug
    ...
    { // iEvent > 0
      ...
      //printf("SmlDataFileReader::eventNextFiducialToGlobal(): fiducialDiff[%d] 0x%x\n", iEvent-1, iFiducialDiff[iEvent-1]); //!!debug      
  ...
  //printf("SmlDataFileReader::eventNextFiducialToGlobal(): FiducialFrom 0x%x  diffAvg %d  To 0x%x  eventPredict %d\n", 
  ...
  while (iEventDiffPredict >= 1)
  {
    ...
    //printf("SmlDataFileReader::eventNextFiducialToGlobal(): new eventPredict %d\n", iEventDiffPredict); //!!debug 
  }
  
  return eventNextFiducialToGlobal(uFiducial, iFromGlobalEvent + iEventDiffPredictOrg + 1, iGlobalEvent);
}

int SmlDataFileReader::eventNextFiducialToCalib(uint32_t uFiducial, int iFromGlobalEvent, int& iCalib, int& iEvent)
{
  iCalib      = -1;
  iEvent      = -1;
...
int SmlDataFileReader::eventNextFiducialToGlobalLinearSearch (uint32_t uFiducial, int iFromGlobalEvent, int& iGlobalEvent)
{
  iGlobalEvent  = -1;
  if (_fdXtcIndex == -1)
    return 1;  
  ...
  for (int iEvent = iFromGlobalEvent; iEvent < _fileHeader.iNumIndex; ++iEvent)
  {
    ...
    if (iError != 0)
    {
      printf("SmlDataFileReader::eventNextFiducialToGlobalLinearSearch(): Cannot get fiducial for global event# %d\n", iEvent);
      return 2;
    }
...
int SmlDataFileReader::detectorList(int& iNumDetector, const ProcInfo*& lDetector) const
{
  iNumDetector = -1;
  if (_fdXtcIndex == -1)
    return 1;
...
int SmlDataFileReader::srcList(int iDetector, int& iNumSrc, const Src*& lSrc) const
{
  if (_fdXtcIndex == -1)
    return 1;
  ...
  {
    printf("SmlDataFileReader::typeList(): Invalid detector id %d (max detId %d)\n",
...
int SmlDataFileReader::typeList(int iDetector, int& iNumType, const TypeId*& lType) const
{
  if (_fdXtcIndex == -1)
    return 1;
...
int SmlDataFileReader::numCalibCycle(int& iNumCalib) const
{
  iNumCalib = -1;
  if (_fdXtcIndex == -1)
    return 1;
...
int SmlDataFileReader::calibCycleList(const CalibNode*& lCalib) const
{
  if (_fdXtcIndex == -1)
    return 1;
...
int SmlDataFileReader::gotoEvent(int iCalib, int iEvent, int64_t& i64offset, int& iGlobalEvent)
{  
  if (_fdXtcIndex == -1)
    return 1;
  ...
  if ( iEvent < 0 )
  {
    if (iCalib == -1)
    {
      printf("SmlDataFileReader::gotoEvent(): cannot goto the beginning of Calib# %d\n",
        iCalib);
      return 3;
    }
    ...
    if ( iError != 0 )
    {
      printf("SmlDataFileReader::gotoEvent(): cannot move index to L1 Event# %d\n",
        calibNode.iL1Index);
      return 4;
    }  
  ...
  if (iError != 0)
  {
    printf("SmlDataFileReader::gotoEvent(): Error converting Calib# %d Event# %d\n",
      iCalib, iEvent);
    return 5;
  }  
...
int SmlDataFileReader::gotoEventInXtc(int iCalib, int iEvent, int fdXtc, int& iGlobalEvent)
{         
  int64_t i64Offset = 0;
  ...
  {
    printf( "SmlDataFileReader::gotoEventAndSeek(): seek xtc failed (expected 0x%Lx, get 0x%Lx), error = %s\n",
...
int SmlDataFileReader::gotoTimeInXtc(uint32_t uSeconds, uint32_t uNanoseconds, int fdXtc, int& iGlobalEvent, bool& bExactMatch, bool& bOvertime)
{  
  iGlobalEvent = -1;
  ...
  if (iError != 0)
  {
    printf("SmlDataFileReader::gotoTime(): cannot move index to L1 event 0x%x (%d)\n",
  ...
  {
    printf( "SmlDataFileReader::gotoEvent(): seek xtc failed (expected 0x%Lx, get 0x%Lx), error = %s\n",
...
int SmlDataFileReader::gotoNextFiducialInXtc(uint32_t uFiducial, int iFromGlobalEvent, int fdXtc, int& iGlobalEvent)
{
  iGlobalEvent = -1;
...
int SmlDataFileReader::time(int iGlobalEvent, uint32_t& uSeconds, uint32_t& uNanoseconds)
{
  uSeconds      = 0;
  uNanoseconds  = 0;
...
int SmlDataFileReader::fiducial(int iGlobalEvent, uint32_t& uFiducial)
{
  uFiducial = 0x1ffff;
  int iError = gotoL1Node(iGlobalEvent);
  if ( iError != 0 )
...
int SmlDataFileReader::offset(int iGlobalEvent, int64_t&  i64Offset)
{
  int iError = gotoL1Node(iGlobalEvent);
  if ( iError != 0 )
    return 1;
...
int SmlDataFileReader::damage(int iGlobalEvent, Damage& damage)
{
  int iError = gotoL1Node(iGlobalEvent);
  if ( iError != 0 )
    return 1;
...
int SmlDataFileReader::detDmgMask(int iGlobalEvent, uint32_t& uMaskDetDmgs)
{
  int iError = gotoL1Node(iGlobalEvent);
  if ( iError != 0 )
    return 1;
...
int SmlDataFileReader::detDataMask(int iGlobalEvent, uint32_t& uMaskDetData)
{
  int iError = gotoL1Node(iGlobalEvent);
  if ( iError != 0 )
    return 1;
...
int SmlDataFileReader::evrEventList(int iGlobalEvent, unsigned int& uNumEvent, const uint8_t*& lEvrEvent)
{
  int iError = gotoL1Node(iGlobalEvent);
  if ( iError != 0 )
    return 1;
...
int SmlDataFileReader::gotoL1Node(int iL1Node)
{
  ...
  {
    printf( "SmlDataFileReader::gotoL1Node(): Invalid L1 Event# %d\n",
      iL1Node );
    return 2;
  }
  ...
  {
    printf("SmlDataFileReader::gotoL1Node(): Seek to CalibCycle offset 0x%Lx failed, result = 0x%Lx\n", 
  ...
  {
    printf( "SmlDataFileReader::gotoL1Node(): Read L1 Node [%d] failed. Read %d bytes (expected %d), error = %s\n",
