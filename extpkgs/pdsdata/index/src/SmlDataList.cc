...
#include "pdsdata/index/SmlDataList.hh"
...
int SmlDataList::startNewNode(const Dgram& dg, int64_t i64Offset, bool& bInvalidData)
{
  bInvalidData = false;
  ...
  if ( _bNewNode )  
  {
    printf( "SmlDataList::startNewNode():: *** Previous new node hasn't been finished\n" );
    finishNode(false);    
    iError = 1;
  }
...
int SmlDataList::updateSegment(const Xtc& xtc)
{
  if ( ! _bNewNode )  
  {
    printf( "SmlDataList::updateSegment():: *** new node hasn't been initialized\n" );
    return 1;
  }
  ...
  {    
    ...
    if ( !rInsert.second )
    {
      printf( "SmlDataList::updateSegment():: Found duplicated segment for "
  ...
  else
  {
    ...
    else
    {
      ...
      {
        printf( "SmlDataList::updateSegment():: *** invalid segment index %d (max value %d)\n", 
...
int SmlDataList::updateSource(const Xtc& xtc, bool& bStopUpdate)
{
  bStopUpdate = false;
  
  if ( ! _bNewNode )  
  {
    printf( "SmlDataList::updateSource():: *** new node hasn't been initialized\n" );
    return 1;
  }
  
  if ( _itCurSeg == _mapSegToId.end() )
  {
    printf( "SmlDataList::updateSource():: Invalid segment\n" );
    return 2;
  }
...
int SmlDataList::updateReporter(const Xtc& xtc, bool& bStopUpdate)
{
  bStopUpdate = false;
  
  if ( ! _bNewNode )  
  {
    printf( "SmlDataList::updateReporter():: *** new node hasn't been initialized\n" );
    return 1;
  }
  
  if ( _itCurSeg == _mapSegToId.end() )
  {
    printf( "SmlDataList::updateReporter():: Invalid segment\n" );
    return 2;
  }
...
int SmlDataList::updateEvr(const Xtc& xtc)
{
...
int SmlDataList::finishNode(bool bPrint)
{
  if ( ! _bNewNode )  
  {
    printf( "SmlDataList::finishNode():: *** new node hasn't been initialized\n" );
    return 1;
  }
...
int SmlDataList::addCalibCycle(int64_t i64Offset, uint32_t uSeconds, uint32_t uNanoseconds)
{  
  _lCalib.push_back(CalibNode(i64Offset,_lNode.size(), uSeconds, uNanoseconds));
  return 0;
}

void SmlDataList::printList(int iVerbose) const
{ 
...
void SmlDataList::printNode(const L1AcceptNode& node, int iSerial) const
{
  char sTimeBuff[128];
...
int SmlDataList::writeToFile(int fdFile) const
{  
...
int SmlDataList::finishPrevSegmentId()
{
  if ( _itCurSeg == _mapSegToId.end() )
    return 0;
...
int SmlDataList::readFromFile(int fdFile)
{  
  SmlDataFileHeaderType fileHeader;
...
SmlDataList::SmlDataList()
{
  reset();
  memset( _sXtcFilename, 0, sizeof(_sXtcFilename) );
}

SmlDataList::SmlDataList(const char* sXtcFilename)
{
  reset();
  setXtcFilename(sXtcFilename);
}

int SmlDataList::reset(bool bClearL1NodeLast)
{
  memset( _sXtcFilename, 0, sizeof(_sXtcFilename) );
  _iNumSegments   = -1;
  _bNewNode       = false;
...
int SmlDataList::setXtcFilename(const char* sXtcFilename)
{
  ...
  {
    printf( "SmlDataList::setXtcFilename(): xtc filename %s is too long (max=%d), "
...
int SmlDataList::getNumNode() const
{
  return _lNode.size();
}

int SmlDataList::getNode(int iNode, L1AcceptNode*& pNode)
{
...
L1AcceptNode& SmlDataList::checkInNode( L1AcceptNode& nodeNew )
{
  if ( _lNode.size() == 0 )
  {      
   ...
   */
  printf( "SmlDataList::checkInNode(): *** event 0x%x need to be inserted at front (total %d elements)\n", 
...
int SmlDataList::writeFileHeader(int fdFile) const
{  
  /*
   * Write main header
   */
  SmlDataFileHeaderType fileHeader(*this);
  int iError = ::write(fdFile, &fileHeader, sizeof(fileHeader) );
  if ( iError == -1 )
  {
    printf( "SmlDataList::writeFileHeader(): write file info header failed (%s)\n", strerror(errno) );
    return 1;
  }
...
int SmlDataList::writeFileL1AcceptList(int fdFile) const 
{
  int iNode = 0;
  ...
  for (TNodeList::const_iterator 
    ...
    iterNode++, iNode++)
  {
    SmlDataFileL1NodeType node(*iterNode);

    int iError = ::write(fdFile, &node, sizeof(node) );
    if ( iError == -1 )
    {
      printf( "SmlDataList::writeFileL1AcceptList(): write node %d failed (%s)\n", iNode, strerror(errno) );              
      return 1;
    }
  }
...
int SmlDataList::writeFileSupplement(int fdFile) const
{
  /*
  ...
  if ( iError == -1 )
  {
    printf( "SmlDataList::writeFileHeader(): write calib node failed (%s)\n", strerror(errno) );    
    return 1;
  }
  ...
  {    
    if ( itMap->second >= (int) _mapEvrToId.size() )
      printf("SmlDataList::writeFileHeader(): Found invalid evr index %d for code %d\n",
  ...
  if ( iError == -1 )
  {
    printf( "SmlDataList::writeFileHeader(): write event code failed (%s)\n", strerror(errno) );    
    return 2;
  }
  ...
  {    
    ...
    {
      printf("SmlDataList::writeFileHeader(): Found invalid segment index %d\n", iIndex);
      continue;
    }
  ...
  for ( int iSeg = 0; iSeg < (int) _mapSegToId.size(); ++iSeg )
  {
    ...
    {
      printf( "SmlDataList::writeFileHeader(): No ProcInfo assigned for segment %d\n",  iSeg);    
      return 3;
    }    
    ...
    if ( iError == -1 )
    {
      printf( "SmlDataList::writeFileHeader(): write proc info failed (%s)\n", strerror(errno) );    
      return 4;
    }
    ...
    {
      printf( "SmlDataList::writeFileHeader(): No src/type data found for segment %d\n",  iSeg);    
      return 5;
    }     
    ...
    if ( iError == -1 )
    {
      printf( "SmlDataList::writeFileHeader(): write NumSrc failed (%s)\n", strerror(errno) );    
      return 6;
    }      
    ...
    if ( iError == -1 )
    {
      printf( "SmlDataList::writeFileHeader(): write src list failed (%s)\n", strerror(errno) );    
      return 7;
    }      
    ...
    if ( iError == -1 )
    {
      printf( "SmlDataList::writeFileHeader(): write type list failed (%s)\n", strerror(errno) );               
      return 8;
    }      
  }    
...
int SmlDataList::readFileHeader(int fdFile, SmlDataFileHeaderType& fileHeader)
{
  /*
   * Read main header
   */
  ...
  if ( iError == -1 )
  {
    printf( "SmlDataList::readFileHeader(): read file header version failed (%s)\n", strerror(errno) );
    return 1;
  }
  
  if ( typeId.id() != TypeId::Id_SmlData  )
  {
    printf( "SmlDataList::readFileHeader(): Unsupported xtc type: %s V%d\n",
      TypeId::name(typeId.id()), typeId.version() );
    return 2;
  }
  ...
  if ( (int) typeId.version() != SmlDataFileHeaderType::iXtcSmlDataVersion )
  {
    if ( (int) typeId.version() == 1 )
    {
      ...
      int iError = ::read(fdFile, &fileHeaderV1, sizeof(SmlDataFileHeaderV1) );
      if ( iError == -1 )
      {
        printf( "SmlDataList::readFileHeader(): read file header failed (%s)\n", strerror(errno) );
        return 1;
      }    
      
      new (&fileHeader) SmlDataFileHeaderType(fileHeaderV1);
    }
    else
    {
      printf( "SmlDataList::readFileHeader(): Unsupported xtc type: %s V%d\n",
...
int SmlDataList::readFileL1AcceptList(int fdFile, int iNumIndex)
{  
  _lNode.clear();  
  for ( int iNode = 0; iNode < iNumIndex; iNode++ )
  {
    SmlDataFileL1NodeType fileNode;

    int iError = ::read(fdFile, &fileNode, sizeof(fileNode) );
    if ( iError == -1 )
    {
      printf( "SmlDataList::readFileL1AcceptList(): read node %d failed (%s)\n", iNode, strerror(errno) );              
      return 1;
    }
...
int SmlDataList::readFileSupplement(int fdFile, const SmlDataFileHeaderType& fileHeader)
{  
  int iError = -1;
  ...
  if (iNumCalib > 0)
  {
    ...
    if ( iError == -1 )
    {
      printf( "SmlDataList::readFileHeader(): read calib failed (%s)\n", strerror(errno) );
      return 1;
    }
  }
  ...
  if ( iNumEvrEvents < 0 )
  {
    printf("SmlDataList::readFileHeader(): Invalid Evr event # %d\n", iNumEvrEvents);
    iNumEvrEvents = 0;
  }
  else if ( iNumEvrEvents > 0 )
  {
    ...
    if ( iError == -1 )
    {
      printf( "SmlDataList::readFileHeader(): read Evr events failed (%s)\n", strerror(errno) );
      return 2;
    }
  ...
  for ( int iSeg = 0; iSeg < _iNumSegments; iSeg++ )
  {
    ...
    if ( iError == -1 )
      printf( "SmlDataList::readFileHeader(): read proc info failed (%s)\n", strerror(errno) );
    ...
    if ( !rInsert.second )
    {
      printf( "SmlDataList::readFileHeader(): read duplicated segment for "
        "ip 0x%x pid 0x%x\n", info.ipAddr(), info.processId() );
    }
    ...
    if ( iError == -1 )
      printf( "SmlDataList::readFileHeader(): read NumSrc failed (%s)\n", strerror(errno) );        
    ...
    if (uNumSrc > 0)
    {
      ...
      if ( iError == -1 )
      {
        printf( "SmlDataList::readFileHeader(): read src list failed (%s)\n", strerror(errno) );
        return 3;
      }
      ...
      if ( iError == -1 )
      {
        printf( "SmlDataList::readFileHeader(): read type list failed (%s)\n", strerror(errno) );
        return 4;
      }
    }        
    segId.bSrcUpdated = true;
...
L1AcceptNode::L1AcceptNode(SmlDataFileL1NodeType& fileNode) :
