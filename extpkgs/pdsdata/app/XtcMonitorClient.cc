...
int XtcMonitorClient::run(const char* tag, int tr_index, int) {
  ...
  while(1) {
    ...
    //
    do {
      int nb = ::recv(myTrFd, (char*)&myMsg, sizeof(myMsg), MSG_WAITALL);
      if (nb < 0) {
        perror("mq_receive buffer");
        return ++error;
      }
      else if (nb == 0) {
        fprintf(stderr, "Message queue has been closed!\n");
        return ++error;

      }
      else if (nb != sizeof(myMsg)){
        fprintf(stderr, "Read %d bytes from queue instead of expected message size of %zu bytes!\n",
                nb, sizeof(myMsg));
        return ++error;
      }
      else {
